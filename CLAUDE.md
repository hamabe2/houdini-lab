...
介在せずに回ることを目指す。現状は**篩から記事の下書きまで無人で通る**ところ
（`unattended.py`）。人が残っているのは「撮れた動画が使えるか」の判断
（`/watch/`）と、記事に固定値の表を足して公開する工程だけ。
...
- `vellum-cloth-restscale.mp4` を公開済み（`bendrestscale` / 1, 100, 1000,
  10000, 100000）。**平らな布では UI 範囲（0〜2）で絵が変わらない**という
  否定形の記事。元メッシュの二面角を hython で実測（最大 0.000001 度）して
  書いた。このパラメータ本来の使い道を見せるには折り目のあるメッシュが要る
- `vellum-cloth-benddampingratio.mp4` を公開済み（`benddampingratio` /
  0, 0.01, 0.1, 1）。**この1本で `/review/` → `shoot.py` → `/watch/` →
  記事 → push の全工程を通した。**`0 → 0.01` が 46.4dB でほぼ同じ絵なのが
  残る難点（既定 0.01 を必ず含めるため避けられない）
- `screening.json`: 273 件 pending / published 3（`bendstiffness` /
  `bendrestscale` / `benddampingratio`）/ screened 7（全部「差なし」）。
  **承認待ちは 0 件**
- **無人モードがある**（`unattended.py` / `unattended.bat`）。篩 → 自動承認 →
  本撮り → 記事の下書きまで通す。**人が絵を見るのは `/watch/`**（動画）に移った。
  下書きは `content/drafts/` に入り、**push しても公開されない**
...
1. **無人モードを実戦で1回通す**（まだ最長3分しか回していない）。`--count` を
   小さく始めて、篩1件あたりの実時間とどこで詰まるかを見る
2. **記事に人しか書けない部分は「固定値の表」だけになった。**「どこを見ると
   違いが分かるか」を書かない方針にしたため（下の「比較コンテンツの作り方」）。
   固定値は `list_parms.py` が実測で持っているので、**どれを載せるかの選別**さえ
   決めれば下書きではなく記事そのものを機械が書ける
3. 下書きから本体の記事へ移す工程（いまは人が書き写している）
4. **`ja`（日本語の説明）が溜まっていない。** 書いてあるのは
   `benddampingratio` の1件だけ。`/watch/` に並ぶたびに書けば増える
...
| | |
|---|---|
| 概要 | そのパラメータが何をするか（2〜3行。検証リストの `ja`） |
| 振った値と測った事実 | 実効値・既定値・有効域・実効値の倍率 |
| 固定値 | この動画で動かさなかった他のパラメータ |

**「どこを見ると違いが分かるか」は書かない。** かつては書いていた（`bendstiffness`
の「下端と左下の角を見る」）が、それは動画がやる仕事で、読者は自分の見たい
ところを見る。**この一行を落としたことで、記事に人しか書けない部分が無くなった**
（＝無人モードが下書きではなく記事そのものを作れる）。
...
- **時間を食うのは篩。本撮りではない。** 実測:
  ...
  | | |
  |---|---|
  | 本撮り1本（5値 x 48フレーム） | **40 秒**（job.json → result.json） |
  | 篩1件 | **1〜23 分**。梯子の段数で決まる |

  梯子が伸びない候補（既定値が `0` で 10^k を掛けても動かないもの）は1分足らずで
  「差なし」と出る。伸びる候補は10段前後を frame 24 まで sim するので20分を超える。
  **`--hours` は篩にも渡す**（`screen_loop.py --minutes`）。本撮りの前でしか
  見ていなかった頃は、`--hours 4` と書いても篩が4時間を超えて走り続けた
- **Ctrl+C / Ctrl+Break でいつ止めてもよい。** 工程ごとに検証リストへ書き戻して
  あるので、止めた時点までは確定している。もう一度回せば止めた候補から続く
- **`--hours` を過ぎても本撮りには入らない。** 承認済みは残るので次に `shoot.py`
...
### 止め方（Ctrl+C と Ctrl+Break は別物）

**ウィンドウを ✕ で閉じない。** Python とその子が強制終了され、起動した
Houdini が残って **Apprentice のライセンスを掴んだままになる**（次の起動が
原因の分かりにくい失敗をする）。

**`Ctrl+C` か `Ctrl+Break` で止める。** どちらでも後始末が走る。

- Windows のコンソールはこの2つを別のシグナルとして投げる。`Ctrl+C` は
  `SIGINT` で Python が `KeyboardInterrupt` にするが、**`Ctrl+Break` は
  `SIGBREAK` で、既定ハンドラがプロセスを即座に殺す**（`try/finally` も
  `except` も通らない。実測で終了コード 0xC000013A = STATUS_CONTROL_C_EXIT）。
  `tools/_signals.py` の `handle_break()` が両方を同じ経路に乗せている
- **ハンドラは子プロセスに継承されない。** 止まってほしいプロセスそれぞれが
  `handle_break()` を呼ぶ（`unattended` / `screen_loop` / `shoot`）
- **起動した `houdini.exe` は必ず終わらせる**（`flipbook.kill_houdini`）。
  `launch()` の `finally` にあるので、正常終了でも中断でも通る。
  Popen を terminate するだけでは足りない（Houdini は自分を別プロセスとして
  立て直すため）
- **Ctrl+C は失敗として数えない。** 人が止めたことは候補の性質ではないので、
  `error_count` を上げると次から飛ばされるようになってしまう

実測で確認済み: Houdini が動いている最中に中断しても、終了コード 130 で
まとめが出て、`houdini.exe` は3秒後に 0 個になる。
