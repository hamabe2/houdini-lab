...
- **撮影経路は2つある。入出力は同じで、絵の作り方だけが違う。**
  ...
  - **アンチエイリアスは明示的に固定している。** 既定のままだとビューポートの
    `sceneAntialias`（実測で 4）と `flipbookAntialias.UseViewportSetting` が使われ、
    その機械の表示設定が出力にそのまま出る。同じコマンドで絵が変わっては
    比較素材にならないので、`--aa`（既定 8）と `HighQuality` で固定する。
