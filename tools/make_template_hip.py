...
def build() -> None:
    ...
    # --- ライトは置かない ----------------------------------------------------
    # **照明はビューポート側で決める**（右のツールバーのライト切り替え。
    # `_hou_flipbook.set_lighting()` が `--lighting` で設定する）。
    #
    # 以前は distant light を1灯置いていたが、平板な絵にしかならなかった。
    # ビューポートのライトは陰影が付くうえ、シーンから照明の要素が消えるので
    # 「パラメータの差」以外が絵に混ざる余地も減る。
    #
    # **副作用: OpenGL ROP 経路（sweep.py）は真っ黒になる。** あちらは
    # ライト0灯だと落ちる（ヘッドライトの自動化が効かない）。撮影は
    # flipbook 経路に一本化しているので許容する。
